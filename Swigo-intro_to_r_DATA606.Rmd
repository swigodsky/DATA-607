...
Starting in 1629, there is an upward trend in the number of girls that were baptized.  However there is a signficant dip beginning around 1640.  The numer of girls baptized begins to increase again starting around 1660.  The number of girls baptized returns to its previous level a little before 1670 and continues to increase until aroudn 1690, when the number of girls baptized plataeus.
...
```{r plot-proportion of boys vs time, eval=TRUE}
...
```{r present-data, eval=TRUE}
present
```

The years are from 1940-2002.
The data set has 63 rows and 3 variables.
The column names are year, boys and girls.

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?

The number of births from 1940-2002 are signficantly larger than those Arbuthnot recorded. The more recent values are hundreds of times greater than the older values.
...
```{r plot-boys to girls ratio for each year}
plot(present$year, present$boys/present$girls)
plot(arbuthnot$year, arbuthnot$boys/arbuthnot$girls)

```
In the present data set, the ratio of boys to girls is always greater than 1, indicating that there are more boys being born than girls.  This is similar to the data Arbuthnot recorded in which the ratio of boys to girls was also greater than 1.
...
```{r max-num-births, eval=TRUE}
num <- which.max(present$boys+present$girls)
```
There were the most births in the year `r present$year[num]`.
